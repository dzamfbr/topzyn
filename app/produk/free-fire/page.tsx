...
const MLBB_TARGET_STORAGE_PREFIX = "topzyn_ff_last_target";
...
export default function FreeFireProductPage() {
  ...
  const summaryPromoText = promoState.code
    ...
    : "-";
  const isReadyToPay = Boolean(selectedItem && gameUserId.trim());
  ...
  useEffect(() => {
    ...
    const loadCatalog = async () => {
      ...
      try {
        const response = await fetch("/api/ff/catalog", {
          method: "GET",
          cache: "no-store",
        });
        ...
        if (!disposed) {
          setCatalogError(
            error instanceof Error
              ? error.message
              : "Gagal memuat katalog Free Fire.",
          );
        }
      } finally {
  ...
  const validateForm = (): boolean => {
    if (!gameUserId.trim()) {
      showTopNotice("Lengkapi ID Game terlebih dahulu.");
      return false;
    }
    if (!selectedItem) {
        ...
        <div className="flex h-[34px] items-center overflow-hidden border-t border-white/35 bg-[#293275] md:h-10">
          <div className="inline-block whitespace-nowrap pl-[100%] [animation:runningText_15s_linear_infinite]">
            <span className="inline-block px-5 text-sm font-bold text-white md:px-7 md:text-base">
              Top up Free Fire cepat dan aman di TopZyn. Pilih nominal, bayar
              QRIS, langsung masuk!
            </span>
          </div>
        </div>
        ...
        <section className="mx-auto mt-6 max-w-6xl sm:mt-7">
          <article className="relative overflow-hidden rounded-2xl border border-white/40 bg-[url('/images/topzyn/bg.png')] bg-cover bg-center p-4 text-white shadow-[0_24px_50px_rgba(41,50,117,0.32)] sm:p-6 md:p-7">
            ...
            <div className="relative grid items-center gap-3 sm:gap-4 md:grid-cols-[auto,1fr]">
              <FallbackImage
                src="/images/topzyn/logos/topzyn-logo-free-fire.png"
                alt="Free Fire"
        ...
        <section className="mx-auto mt-5 grid max-w-6xl gap-4 pb-8 sm:mt-6 sm:gap-6 sm:pb-10 lg:grid-cols-[minmax(260px,340px)_1fr] lg:items-start">
          <aside className="h-fit px-1">
            ...
            <ol className="mt-3 list-decimal space-y-2.5 pl-5 text-xs leading-relaxed text-slate-600 sm:text-sm">
              <li>
                Masukkan ID Game Free Fire dengan benar sesuai akun kamu.
                Pastikan tidak ada salah ketik agar item masuk ke akun yang
                tepat.
              </li>
              <li>
          ...
          <div className="space-y-5">
            <section className={sectionCardClass}>
              ...
              <p className="text-xs text-slate-500 sm:text-sm">
                Cek kembali ID game sebelum lanjut order.
              </p>
              <div className="mt-4">
                <input
                  type="text"
                  placeholder="ID Game Free Fire"
                  value={gameUserId}
